...
By default this generates Bash commands for a Debian system, but you can specify a different shell or OS in the prompt itself (or [modify the default value](#modifying-default-values)):
...
- `DEFAULT_MODEL_ARGS`: the default model arguments used by all commands if not overridden.
